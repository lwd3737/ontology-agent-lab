...
const queryResultToOntologyTranslationEvaluator = ({
  ...
  const pipelineResultDetails = pipelineResult.map(
    (pipelineStepResult, stepIndex) => {
      const queryDSLPipelineStep = queryDSL.pipeline[stepIndex];
      ...
      const instanceDetails = pipelineStepResult.objectInstances.reduce(
        (result, objectInstance, objectInstanceIndex) => {
